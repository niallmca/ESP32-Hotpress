...
#define motd "Hotpress 4-June-2019"
...
void setup(void) {
  ...
  timer.setInterval(5000, getTemps);
...
void getTemps()
{
 ...
 delay(dhtBathroom.getMinimumSamplingPeriod());
   ...
   if ((bh > 5.00 )  && (bh <= 100)) { BathroomHumid = bh; }
   bt = dhtBathroom.getTemperature();
   if ((bt > 5) && (bt < 50)) { BathroomTemp = bt; }
...
void doFailsafe()
{
  ...
  if (immersionTemp > 70)
    {
      Serial.println("boiler > 70 deg C ...turning off immersion");
